...
class UNPATTI_Gallery_Grid extends \Elementor\Widget_Base {

    public function get_name() { return 'unpatti_gallery_grid'; }
    public function get_title() { return __( 'Gallery Grid', 'unpatti-academic' ); }
    ...
    protected function register_controls() {
        ...
        $this->add_control( 'kategori', [
            ...
            'description' => __( 'Filter by gallery taxonomy slug. Leave empty for all.', 'unpatti-academic' ),
        ] );
    ...
    protected function render() {
        ...
        $id       = 'unpatti-gallery-' . $this->get_id();
        ...
        <style>
            ...
            #<?php echo esc_attr( $id ); ?> .unpatti-gallery-item {
                position: relative; overflow: hidden; border-radius: 6px;
                aspect-ratio: 1; cursor: pointer;
            }
            #<?php echo esc_attr( $id ); ?> .unpatti-gallery-item img {
                width: 100%; height: 100%; object-fit: cover; transition: transform .3s;
            }
            #<?php echo esc_attr( $id ); ?> .unpatti-gallery-item:hover img { transform: scale(1.08); }
            #<?php echo esc_attr( $id ); ?> .unpatti-gallery-overlay {
            ...
            #<?php echo esc_attr( $id ); ?> .unpatti-gallery-item:hover .unpatti-gallery-overlay { opacity: 1; }
        ...
        <div id="<?php echo esc_attr( $id ); ?>">
            <?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
                <a href="<?php the_permalink(); ?>" class="unpatti-gallery-item">
                    <?php if ( has_post_thumbnail() ) : ?>
                        <?php the_post_thumbnail( 'unpatti-card' ); ?>
                    <?php endif; ?>
                    <div class="unpatti-gallery-overlay">
                        <span><?php the_title(); ?></span>
                    </div>
                </a>
            <?php endwhile; wp_reset_postdata(); else : ?>
                <p><?php esc_html_e( 'No gallery items found.', 'unpatti-academic' ); ?></p>
            <?php endif; ?>
        </div>
        <?php
    }
